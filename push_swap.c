...
/*   Updated: 2022/10/29 12:47:39 by aelsiddi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	while(av[ps.main_i] != NULL)
	{
		...
		{
			if (!(find_duplicate(&ps.a, ft_atoi(ps.split_array[ps.main_j],&ps), &ps)))
				exit_error(6, &ps);
	...
	// print(ps.a);
	// fill_it(ac, ps.split_array, &ps.a);
	// printf("ac = %d\n", ac);
	arg_handler(ac, &ps.a, &ps.b);
	// print(ps.a);
	// printf(" 2ed ac = %d\n", ac);
	// whatever(ps.split_array,ac);
